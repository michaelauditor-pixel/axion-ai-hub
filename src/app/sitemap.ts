...
import { toolDefinitions } from "./tools/data";
...
export default function sitemap():MetadataRoute.Sitemap{
  ...
  const toolEntries=toolDefinitions.map(item=>({url:`${SITE_URL}/tools/${item.slug}`,lastModified:now,changeFrequency:"weekly" as const,priority:.9}));
  const useCaseEntries=useCases.map(item=>({url:`${SITE_URL}/use-cases/${item.slug}`,lastModified:now,changeFrequency:"monthly" as const,priority:.82}));
  return [...coreEntries,...toolEntries,...useCaseEntries];
}
